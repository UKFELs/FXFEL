...
output_file=tables.open_file(file_name_base+'.vsh5','w')

# Create hdf5 file

particleName="Particles"
# Save the array into hdf5 file
ParticleGroup=output_file.create_array('/',particleName,x_px_y_py_z_pz_NE)
...
varsGroup=output_file.create_group('/',particleName+'_t','Time derived from Z')
varsGroup._v_attrs.vsType='vsVars'
varsGroup._v_attrs.__setattr__(particleName+'_t','z/2.998e8')
varsGroup=output_file.create_group('/',particleName+'_dz','')
varsGroup._v_attrs.vsType='vsVars'
varsGroup._v_attrs.__setattr__(particleName+'_dz','z-'+str(np.average(norm_z)))
varsGroup=output_file.create_group('/',particleName+'_dt','T minus average T')
varsGroup._v_attrs.vsType='vsVars'
varsGroup._v_attrs.__setattr__(particleName+'_dt',particleName+'_t-'+str(np.average(norm_z))+'/2.998e8')
varsGroup=output_file.create_group('/',particleName+'_dp','Momentum as a proportion of average momentum')
varsGroup._v_attrs.vsType='vsVars'
aveMom=np.average(np.sqrt(np.add(np.add(np.square(norm_px),np.square(norm_py)),np.square(norm_pz))))
varsGroup._v_attrs.__setattr__(particleName+'_dp','(sqrt(px^2+py^2+pz^2)-'+str(aveMom)+')/'+str(aveMom))
