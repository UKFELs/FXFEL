...
NumParticlesInSlice=NumberOfParticlesPerSlice
...
#density_Y=np.linspace(0, 1, Num_Of_Target_Particles)
#density_X=np.linspace(0, 1, Num_Of_Target_Particles)
print max(mB_X)
print max(mB_Y)
...
#    f_Y = UnivariateSpline(x0_Y, y0_Y)
#    f_X = UnivariateSpline(x0_X, y0_X)


    xnew_Z = np.linspace(np.min(m_Z),np.max(m_Z),binnumber)
    xnew_Y = np.linspace(np.min(mB_Y),np.max(mB_Y),binnumber)
    xnew_X = np.linspace(np.min(mB_X),np.max(mB_X),binnumber)
    ...
    def remove_duplicates(li):
        ...
        for e in li:
            if e not in my_set:
                ...
                my_set.add(e)
        return res
#    cumulative_nq_Z=sorted(set(cumulative_Z), key=lambda x: cumulative_Z.index(x))
#    cumulative_nq_Y=sorted(set(cumulative_Y), key=lambda x: cumulative_Y.index(x))
#    cumulative_nq_X=sorted(set(cumulative_X), key=lambda x: cumulative_X.index(x))
    
    cumulative_nq_Z=remove_duplicates(cumulative_Z)
    cumulative_nq_Y=remove_duplicates(cumulative_Y)
    cumulative_nq_X=remove_duplicates(cumulative_X)    
#    cumulative_nq_Z=sorted(set(cumulative_Z))
#    cumulative_nq_Y=sorted(set(cumulative_Y))
#    cumulative_nq_X=sorted(set(cumulative_X))

#    print len(cumulative_nq_X),len(cumulative_nq_Y),len(cumulative_nq_Z)

    xx_0_Z=np.linspace(np.min(m_Z),np.max(m_Z),len(cumulative_nq_Z))
    xx_0_Y=np.linspace(np.min(mB_Y),np.max(mB_Y),len(cumulative_nq_Y))
    xx_0_X=np.linspace(np.min(mB_X),np.max(mB_X),len(cumulative_nq_X))

    ff_Z = interp1d(cumulative_nq_Z, xx_0_Z)
    ff_Y = UnivariateSpline(cumulative_nq_Y, xx_0_Y)
    ff_X = UnivariateSpline(cumulative_nq_X, xx_0_X)
...
#   if stepnumber==1:    
        #density_Y=np.linspace(min(cumulative_nq_Y), max(cumulative_nq_Y), Num_Of_Target_Particles)
        #density_X=np.linspace(min(cumulative_nq_X), max(cumulative_nq_X), Num_Of_Target_Particles)
#        density_Y=np.random.uniform(low=0, high=1, size=(Num_Of_Target_Particles))
#        density_X=np.random.uniform(low=0, high=1, size=(Num_Of_Target_Particles))
#    else:
#        print 'fail'
...
#plt.xlabel('Z')
#plt.ylabel('Density')
#plt.hist(Full_Z,weights=No_Particles_Per_Record,bins=binnumber*TotalNumberOfSteps)
