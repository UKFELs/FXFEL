...
NumberOfParticlesPerSlice=1000
TotalNumberOfSteps=int(len(mB_X)/NumberOfParticlesPerSlice)
...
for stepnumber in range(0,TotalNumberOfSteps):
...
#   else:            
    ...
    Hy, edges_Y = np.histogramdd(m_Y, bins = binnumber*10,normed=False,weights=m_WGHT.flat)
    Hx, edges_X = np.histogramdd(m_X, bins = binnumber*10,normed=False,weights=m_WGHT.flat)
    ...
    x0_Y = np.linspace(np.min(m_Y),np.max(m_Y),binnumber*10)
    x0_X = np.linspace(np.min(m_X),np.max(m_X),binnumber*10)
    
    from scipy import interpolate
    from scipy.interpolate import interp1d
    f_Z = interpolate.InterpolatedUnivariateSpline(x0_Z, y0_Z)
    f_Y = interp1d(x0_Y, y0_Y)
    f_X = interp1d(x0_X, y0_X)


    xnew_Z = np.linspace(np.min(m_Z),np.max(m_Z),binnumber)
    xnew_Y = np.linspace(np.min(m_Y),np.max(m_Y),binnumber*10)
    xnew_X = np.linspace(np.min(m_X),np.max(m_X),binnumber*10)
...
#    density_Z=np.random.uniform(low=min(cumulative_nq_Z), high=max(cumulative_nq_Z), size=(Num_Of_Target_Particles))    
    ...
    print 'z_density_Z length = ',len(z_density_Z)
